...
static void gvm_init_xsave(CPUX86State *env)
{
    if (has_xsave) {
        env->xsave_buf_len = sizeof(struct gvm_xsave);
    } else {
        return;
    }
     ...
     */
    assert(gvm_arch_get_supported_cpuid(gvm_state, 0xd, 0, R_ECX) <=
           env->xsave_buf_len);
}
...
static int gvm_put_xsave(X86CPU *cpu)
{
    ...
    uint32_t xsave_len = env->xsave_buf_len;

    if (!has_xsave) {
        return gvm_put_fpu(cpu);
    }
    x86_cpu_xsave_all_areas(cpu, xsave, xsave_len);
